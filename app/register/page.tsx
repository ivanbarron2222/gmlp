...
import { AlertCircle, Camera, CheckCircle2, QrCode, Upload, X } from 'lucide-react';
...
export default function PatientRegistrationPage() {
  ...
  const startCamera = async () => {
    try {
      ...
      setSubmitError('Unable to open the camera. Check browser permission or upload a photo instead.');
    }
  };
  ...
  const uploadProfilePhoto = async (registration: PendingRegistration) => {
    if (!profilePhoto) return;
    const photoForm = new FormData();
    photoForm.set('registrationCode', registration.registrationCode);
    photoForm.set('birthDate', registration.birthDate);
    photoForm.set('photo', profilePhoto);
    const response = await fetch('/api/public/patient-photo', { method: 'POST', body: photoForm });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await uploadProfilePhoto(registration);
      } catch (error) {
        setSubmitError(
          error instanceof Error
            ? `Registration succeeded, but the profile photo could not be saved: ${error.message}`
            : 'Registration succeeded, but the profile photo could not be saved.'
        );
      }
      stopCamera();
      clearProfilePhoto();
              ...
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Capture a clear profile photo or upload an existing image.
                  </p>
                </div>
                {profilePhotoPreview && (
